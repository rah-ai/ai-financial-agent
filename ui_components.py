...
def apply_custom_css(theme):
    """
    Applies a hyper-stylized CSS with a new, robust theme-toggling mechanism.
    This is the definitive fix for the light/dark mode issue.
    """
    
    # Start with a base of all common styles, animations, and fonts
    # This block is identical to the one you liked.
    css = """
        /* --- FONTS & ANIMATIONS --- */
    ...
    if theme == "light":
        theme_css = """
            ...
            body, .stApp, h1, h2, h3, p, label, div[data-testid="stMarkdownContainer"], .st-emotion-cache-16txtl3 {
        ...
        theme_css = """
            .stApp {
                background: linear-gradient(-45deg, #0f0c29, #302b63, #24243e, #004e92);
                background-size: 400% 400%;
                animation: background_pan 15s ease-in-out infinite;
            }
             body, .stApp, h1, h2, h3, p, label, div[data-testid="stMarkdownContainer"], .st-emotion-cache-16txtl3 {
    ...
    # Combine common and theme-specific CSS into one block and inject it.
    st.markdown(f"<style>{css}{theme_css}</style>", unsafe_allow_html=True)


# ===================================================================
# ALL OTHER FUNCTIONS BELOW ARE PRESERVED EXACTLY AS THEY WERE.
...
def display_profile_sidebar(lang):
    ...
    with st.sidebar:
        ...
        occupations = ["Farmer", "Student", "Business Owner", "Unemployed", "Other"]
